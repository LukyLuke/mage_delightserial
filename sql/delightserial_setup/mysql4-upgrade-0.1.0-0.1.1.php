...
CREATE TABLE `{$installer->getTable('delightserial/numbers')}`(
  ...
  PRIMARY KEY `DELIGHTSERIAL_AV_PK` (`serial_id`),
  KEY `DELIGHTSERIAL_AV_GROUP` (`group_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
");

$conn->addConstraint(
    'FK_DELIGHTSERIAL_AVNUMBERS_GROUP', $installer->getTable('delightserial/numbers'), 'group_id', $installer->getTable('delightserial/groups'), 'entity_id'
);
